...
if(!isset($_SESSION['user_id'])) {
    header("Location: login_komunikasi.php");
    exit();
}

$questions = [
    "Saya kurang suka dengan orang yang latah saat berbicara.",
    "Saya tidak terlalu bisa memahami perasaan orang lain.",
    "Saya dapat menangkap inti masalah yang diceritakan orang lain.",
    "Saya dapat menegaskan inti masalah teman saya saat dia mulai bercerita berputar-putar.",
    "Saya suka memaksakan keputusan saya pada orang lain.",
    "Saya kurang yakin dengan pendapat yang saya sampaikan.",
    "Saya merasakan kebahagiaan yang diceritakan oleh orang lain.",
    "Saya sering membantu teman dalam memahami masalah yang dia hadapi.",
    "Saya selalu bersikap positif terhadap kekurangan orang lain.",
    "Saya selalu jujur menceritakan masalah saya pada orang lain .",
    "Saya dapat menjaga rahasia teman dengan baik.",
    "Saya akan bersikap rileks saat mendengarkan cerita orang.",
    "Saya suka memotong pembicaraan orang lain sebelum selesai bicara.",
    "Saya bersikap acuh tak acuh saat orang lain bercerita.",
    "Saya senang memotivasi teman saat dia putus asa.",
    "Saat orang bercerita saya akan memandang matanya.",
    "Saya selalu bersungguh-sungguh mendengarkan cerita orang lain.",
    "Saya akan duduk dengan nyaman saat mendengarkan cerita orang.",
    "Saya akan mengangguk saat setuju dengan pendapat orang lain.",
    "Saat teman sedih dan tidak dapat bercerita saya akan memegang tangannya untuk memberi dukungan.",
    "Saya tidak suka dengan orang yang berbicara dengan cepat."
];
...
if (!isset($_SESSION['confidence_quiz_answers'])) {
    $_SESSION['confidence_quiz_answers'] = [];
}
...
if (isset($_POST['submit'])) {
    ...
    foreach ($questions as $key => $question) {
        if (isset($_POST['q_' . $key])) {
            $_SESSION['confidence_quiz_answers'][$key] = $_POST['q_' . $key];
        }
    }
    ...
    $unansweredQuestions = [];
    foreach ($questions as $key => $question) {
        if (!isset($_SESSION['confidence_quiz_answers'][$key]) || $_SESSION['confidence_quiz_answers'][$key] === '') {
    ...
    if (empty($unansweredQuestions)) {
        ...
        $totalScore = 0;
        foreach ($_SESSION['confidence_quiz_answers'] as $key => $answer) {
            $totalScore += $options[$answer];
        }
        
        $category = "";
        if ($totalScore >= 80 && $totalScore <= 100) {
            $category = "Keterampilan Komunikasi sangat tinggi";
        } elseif ($totalScore >= 60 && $totalScore <= 79) {
            $category = "Keterampilan Komunikasi tinggi";
        } elseif ($totalScore >= 40 && $totalScore <= 59) {
            $category = "Keterampilan Komunikasi sedang";
        } elseif ($totalScore >= 20 && $totalScore <= 39) {
            $category = "Keterampilan Komunikasi rendah";
        } else {
            $category = "Keterampilan Komunikasi sangat rendah";
        }


        // Koneksi ke database
        $conn = new mysqli("localhost", "root", "", "keterampilan_komunikasi");
        if ($conn->connect_error) {
            ...
            if ($stmt->execute()) {
                $resultHTML = "<div class='result-container'>
                    ...
                    <p><a href='logout_komunikasi.php' style='color: #fff; text-decoration: none;'>Logout</a></p>
                </div>";
                $showForm = false;
                
                // Reset session untuk quiz
                unset($_SESSION['confidence_quiz_answers']);
            } else {
...
<head>
    ...
    <title>Kuis Keterampilan Komunikasi</title>
    <style>
        body {
...
<body>
    <div class="quiz-container">
        <h1>Kuis Keterampilan Komunikasi</h1>
                ...
                <div class="options">
                    ...
                    <div>
                        <input type="radio" name="q_<?php echo $key; ?>" id="q_<?php echo $key; ?>_<?php echo $option; ?>" value="<?php echo $option; ?>" 
                            <?php if (isset($_SESSION['confidence_quiz_answers'][$key]) && $_SESSION['confidence_quiz_answers'][$key] === $option) echo 'checked'; ?>>
