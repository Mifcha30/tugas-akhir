...
if(!isset($_SESSION['user_id'])) {
    header("Location: login_penyesuaian.php");
    exit();
}

$questions = [
    "Saya senang mendapatkan banyak pengalaman.",
    "Saya percaya diri terhadap kondisi saya saat ini.",
    "Saya memiliki hubungan pertemanan yang baik.",
    "Saya mampu bersosialisasi dan menjalin hubungan baik dengan orang lain.",
    "Saya percaya diri dapat menyelesaikan suatu pekerjaan dengan baik.",
    "Saya tipe orang yang mudah bergaul.",
    "Saya belum bisa mengontrol kebiasaan-kebiasaan buruk yang orang lain tidak mengetahuinya.",
    "Saya senang berada lingkungan baru.",
    "Lebih baik saya berada di kost atau asrama seharian daripada saya harus bertemu dengan teman-teman.",
    "Saya merasa tidak dekat dengan teman-teman saya, meskipun setiap hari bersama mereka.",
    "Saya tidak suka berada di tempat ramai.",
    "Saya selalu optimis dengan kemampuan saya.",
    "Saya merasa minder ketika mengetahui kelebihan orang lain.",
    "Saya tidak dapat menerima pendapat orang lain yang tidak sependapat dengan saya.",
    "Saya mampu menerima pendapat orang lain.",
    "Saya mampu bangkit dari keterpurukan karena saya yakin pada diri saya.",
    "Saya berinteraksi dengan orang lain tanpa melihat perbedaan.",
    "Saya merasa tidak nyaman berada diantara teman-teman saya.",
    ...
    "Saya sering bertegur sapa dengan teman kos/kontrakan setiap bertemu.",
    "Saya diterima dengan baik oleh orang-orang disekitar saya.",
    "Saya bertanggung jawab ketika saya berbuat kesalahan.",
    "Saya kurang mampu bertanggung jawab ketika saya berbuat kesalahan."
];
...
if (isset($_POST['submit'])) {
    ...
    if (empty($unansweredQuestions)) {
        ...
        if ($totalScore >= 80 && $totalScore <= 100) {
            $category = "Penyesuaian Diri sangat tinggi";
        } elseif ($totalScore >= 60 && $totalScore <= 79) {
            $category = "Penyesuaian Diri tinggi";
        } elseif ($totalScore >= 40 && $totalScore <= 59) {
            $category = "Penyesuaian Diri sedang";
        } elseif ($totalScore >= 20 && $totalScore <= 39) {
            $category = "Penyesuaian Diri rendah";
        } else {
            $category = "Penyesuaian Diri sangat rendah";
        }

        // Koneksi ke database
        $conn = new mysqli("localhost", "root", "", "penyesuaian_diri");
        if ($conn->connect_error) {
            ...
            if ($stmt->execute()) {
                $resultHTML = "<div class='result-container'>
                    ...
                    <p><a href='logout_penyesuaian.php' style='color: #fff; text-decoration: none;'>Logout</a></p>
                </div>";
                $showForm = false;
...
<head>
    ...
    <title>Kuis Penyesuaian Diri</title>
    <style>
        body {
